...
"""
channels = ["https://www.youtube.com/@FullPlaythroughs/"]
...
for idx, chan in enumerate(channels):
    ...
    for jdx, video in tqdm(enumerate(c.videos, start=count), total=num_videos):
        ...
        audio_stream = None
        # only use videos with the keywords in title
        if all(keyword in video.title.lower() for keyword in ["no commentary", "full game", "gameplay walkthrough"]):
